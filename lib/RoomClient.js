...
const VIDEO_CONSTRAINS_DEFAULT =
{
...
const VIDEO_SIMULCAST_ENCODINGS_DEFAULT =
[
...
export default class RoomClient
{
	...
	)
	{
		...
		// Protoo URL.
		// @type {String}
		this._protooUrl = getProtooUrl({ media_server_wss, roomId, peerId, forceH264, forceVP9 });
		...
		this._webcam =
		{
			device     : args.camDeviceId ? { deviceId: args.camDeviceId } : null,
			resolution : 'hd'
		};

		this._mic =
		{
			device : args.micDeviceId ? { deviceId: args.micDeviceId } : null
		};
	...
	async enableMic()
	{
		...
		try
		{
			if (!this._externalVideo)
			{
				...
				const deviceId = this._mic.device ? this._mic.device.deviceId : null;
	...
	async changeMic()
	{
		...
		{
			const deviceId =
				this._mic.device ? this._mic.device.deviceId : undefined;

			// this._mic.device = this._mic.device.deviceId = deviceId;
